...
export default class PageSet {
    ...
    public static openPage() {
        console.log('push undefined?', PageSet.pusharr, PageSet.nowNode)
        for (let i = PageSet.pusharr.length - 1; i >= 0; i--) {
             if (!PageSet.pusharr[i]) continue;
    ...
    if (PageSet.nowNode && PageSet.pusharr.indexOf(PageSet.nowNode) == -1) {
